...
async def clone(event):
    ...
    old_bio = (await event.client(functions.users.GetFullUserRequest("me"))).about

    
    first_name = replied_user.first_name or ""
    last_name = replied_user.last_name or ""
    full_user = await event.client(functions.users.GetFullUserRequest(replied_user.id))
    ...
    if profile_pic:
        ...
        await event.reply("⚠️ Profil şəkli tapılmadı. Sadəcə ad və bio klonlandı.")
