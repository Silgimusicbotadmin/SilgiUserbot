...
class PMMessageFilters(BASE):
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, PMMessageFilters) and self.user_id == other.user_id
...
def get_pm_filter(user_id, keyword):
    try:
        return SESSION.query(PMMessageFilters).get((str(user_id), keyword))
    finally:
        SESSION.close()
def get_pm_filters(user_id):
    try:
        return SESSION.query(PMMessageFilters).filter(
            PMMessageFilters.user_id == str(user_id)).all()
    finally:
        SESSION.close()
def add_pm_filter(user_id, keyword, reply, f_mesg_id):
    to_check = get_pm_filter(user_id, keyword)
    if not to_check:
        adder = PMMessageFilters(str(user_id), keyword, reply, f_mesg_id)
        SESSION.add(adder)
        SESSION.commit()
        return True
    else:
        rem = SESSION.query(PMMessageFilters).get((str(user_id), keyword))
        SESSION.delete(rem)
        SESSION.commit()
        adder = PMMessageFilters(str(user_id), keyword, reply, f_mesg_id)
        SESSION.add(adder)
