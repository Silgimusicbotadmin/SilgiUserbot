...
async def add_pm_filter_handler(event):
    ...
    user_id = str(event.client.uid)  # Filteri saxlayan user ID
    add_pm_filter(user_id, filter_name, response, msg_id)
...
async def pm_filter_handler(event):
    ...
    user_id = str(event.client.uid)  # Ancaq öz filterlərimiz işləsin
    filters = get_pm_filters(user_id)
    if not filters:
        return
...
async def remove_pm_filter_handler(event):
    ...
    user_id = str(event.client.uid)
    if remove_pm_filter(user_id, filter_name):
